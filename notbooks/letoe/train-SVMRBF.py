...
def fun(args):
    ...
    df_result.to_pickle('./_pickles/partial/train{}-eval{}-alg{}.pickle'.format(len(others_coll), 
                                                                       collection_eval,
                                                                       kiii))
...
print(arg_instances)
results = Parallel(n_jobs=3,verbose=5)(delayed(fun)(arg) for arg in arg_instances)



for i, (collection_eval, others_coll) in enumerate(arg_instances):
    sub = pd.read_pickle('./_pickles/partial/train{}-eval{}-alg{}.pickle'.format(len(others_coll), collection_eval, kiii))
    if i==0:
        dfres = pd.DataFrame(columns=sub.columns)
    dfres = pd.concat([dfres,sub])
dfres.to_pickle('./_pickles/{}.pickle'.format(kiii))
#display(dfres)
