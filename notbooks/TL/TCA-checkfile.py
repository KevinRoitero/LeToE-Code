...
sys.path.append(os.path.dirname(os.path.expanduser('../src/tca.py')))
from tca import TCA
...
def fun(args):
    ...
    if os.path.isfile('./_pickles/TCA/train{}-eval{}-train.pickle'.format(others_coll[0], collection_eval)  ):
        pass
    else:
        rev_collections = list(reversed(collections))
        ...
        my_tca = TCA(dim=5,kerneltype='rbf', kernelparam=1, mu=1)
        x_src_tca, x_tar_tca, _ = my_tca.fit_transform(x_src=x_train, x_tar=x_test)

        train = pd.DataFrame(x_src_tca)
        test = pd.DataFrame(x_tar_tca)

        train.columns = ['F{}'.format(i+1) for i in range(len(train.columns))]
        test.columns = ['F{}'.format(i+1) for i in range(len(test.columns))]

        train['AP'] = y_train
        test['AP'] = y_test

        train.to_pickle('./_pickles/TCA/train{}-eval{}-train.pickle'.format(others_coll[0], collection_eval))
        test.to_pickle('./_pickles/TCA/train{}-eval{}-test.pickle'.format(others_coll[0], collection_eval))
