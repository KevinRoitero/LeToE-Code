...
def fun(args):
    ...
    m_test = pd.read_pickle('./_pickles/TCA/train{}-eval{}-test.pickle'.format(others_coll, collection_eval))
    m_train = pd.read_pickle('./_pickles/TCA/train{}-eval{}-test.pickle'.format(others_coll, collection_eval))
    ...
    print(predictions)
    print(y_test)
    print('eval-{} -- pearson: {}'.format(collection_eval, sp.stats.pearsonr(predictions, y_test)[0]  )   )
    ...
    df_result.loc[len(df_result)] = [others_coll,
                                     collection_eval, 'Base', kiii, df_res]
    
    df_result.to_pickle('./_pickles/partial/train{}-eval{}-algTCA{}.pickle'.format(others_coll, 
                                                                       collection_eval,
                                                                       kiii))
